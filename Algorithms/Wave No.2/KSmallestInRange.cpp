...
int kth(Node* R, Node* L, int l, int r, int k) {
    if (l == r) return l;
    ...
    if (k <= leftCount)
        return kth(R->left, L->left, l, mid, k);
    else
        return kth(R->right, L->right, mid + 1, r, k - leftCount);
}
...
int main() {
    ...
    while (q--) {
        ...
        int idx = kth(version[R], version[L - 1], 1, m, k);
        cout << comp[idx - 1] << "\n";
    }
